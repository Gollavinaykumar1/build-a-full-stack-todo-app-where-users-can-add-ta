// src/App.jsx — Generated by VIA for: Build a full stack todo app where users can add tasks with title, priority and due date, mark them as complete and delete them
...
// FIX: handles all backend response shapes:
//   raw array          → [...]
//   {items: [...]}    → most common FastAPI pattern
//   {data: [...]}     → some backends use this key
//   {results: [...]}  → DRF / paginated APIs
//   single object      → {id, title, ...} backend bug — wrap in array so list never breaks
const parseItems = (data) => {
  if (Array.isArray(data)) return data;
  if (data?.items   && Array.isArray(data.items))   return data.items;
  if (data?.data    && Array.isArray(data.data))    return data.data;
  if (data?.results && Array.isArray(data.results)) return data.results;
  // FIX: backend bug — GET /items returns a single object instead of a list.
  // This happens when the backend's get_items() accidentally returns one record.
  // Wrapping in an array lets the UI show what exists rather than an empty list.
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <Link to="/" className="flex items-center gap-2">
              ...
              <span className="font-display font-bold text-gray-900 text-lg">Build A Full Stack Todo App Where Users</span>
            </Link>
            <div className="flex items-center gap-3">
              ...
              <Link to="/new" className="btn-primary text-sm py-2 px-4">+ Create New</Link>
            </div>
          </div>
        </div>
      </nav>
...
function HomePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      // FIX: use parseItems so any response shape works, including single-object backend bug
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-gray-900">Build A Full Stack Todo App Where Users</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <StatCard title="Total" value={stats?.total ?? 0} loading={loading} color="blue" />
        <StatCard title="Active" value={stats?.active ?? 0} loading={loading} color="green" />
        <StatCard title="System" value={"Online"} loading={loading} color="purple" />
      </div>
      <div className="card">
        ...
        {loading ? <Spinner /> : items.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-400 mb-4">No records yet</p>
            <Link to="/new" className="btn-primary inline-block">+ Create New</Link>
          </div>
        ) : (
...
function ItemListPage() {
  ...
  // FIX: extracted so delete handler can call it to refetch after removing an item
  const load = useCallback(() => {
    setLoading(true);
    // FIX: getItems() called with NO params — prevents any stale id leaking into
    // the query string which would hit GET /items/<id> (single-item endpoint)
    // instead of GET /items (list endpoint)
    getItems()
      // FIX: use parseItems — handles array / {items} / {data} / {results} /
      // single-object backend bug (wraps in array so list is never blank)
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // FIX: call load() instead of setItems(p => p.filter(i => i.id !== id))
      // Old manual filter broke when backend uses _id instead of id — deleted item
      // stayed visible in the list because i.id !== id was always true for _id items
      load();
    } catch {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <Link to="/new" className="btn-primary whitespace-nowrap">+ Create New</Link>
        </div>
      </div>
      {filtered.length === 0 ? (
        <div className="card text-center py-20">
          <p className="text-5xl mb-4">✅</p>
          <p className="text-gray-400 text-lg mb-6">No records yet</p>
          <Link to="/new" className="btn-primary inline-block">+ Create New</Link>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50/80 border-b border-gray-100">
              <tr>
                ...
                <th className="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase hidden md:table-cell">Description</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map(item => (
                <tr key={item.id ?? item._id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-gray-500 hidden md:table-cell">{item.description || '—'}</td>
...
function ItemFormPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // FIX: navigate BEFORE toast so ItemListPage mounts fresh with no stale state.
      // This ensures load() calls getItems() cleanly with no leftover params.
      navigate("/items");
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="card">
        <h1 className="text-2xl font-display font-bold text-gray-900 mb-6">Create New Record</h1>
